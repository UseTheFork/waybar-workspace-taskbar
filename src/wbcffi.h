...
extern "C" {
    ...
    typedef struct {
        ...
        /// Queues a request for calling wbcffi_update() on the next GTK main
    ...
    typedef struct {
        ...
        /// string, or the JSON representation of any other JSON object as a
        /// string.
        ///
        /// From ABI version 2 onwards, this is always the JSON representation
    ...
    /// @param config_entries     Flat representation of the module JSON config.
    /// The data only available
    ///                           during wbcffi_init call.
    /// @param config_entries_len Number of entries in `config_entries`
    ///
    /// @return A untyped pointer to module data, NULL if the module failed to
    /// load.
    void *wbcffi_init(
    ...
    /// Called when Waybar receives a POSIX signal and forwards it to each
    /// module
    ///
    /// Optional CFFI function
    ///
